...
namespace travel {
    ...
    public:
        ...
        void getParams(double &max_range, double &min_range, double &resolution, 
                            int &num_iter, int &num_lpr, int &num_min_pts, double &th_seeds, 
                            double &th_dist, double &th_outlier, double &th_normal, double &th_weight, 
                            double &th_lcc_normal_similiarity, double &th_lcc_planar_model_dist, double &th_obstacle,
                            bool &refine_mode, bool &visualization)
        {
            max_range = MAX_RANGE_;
            min_range = MIN_RANGE_;
            resolution = TGF_RESOLUTION_;
            num_iter = NUM_ITER_;
            num_lpr = NUM_LRP_;
            num_min_pts = NUM_MIN_POINTS_;
            th_seeds = TH_SEEDS_;
            th_dist = TH_DIST_;
            th_outlier = TH_OUTLIER_;
            th_normal = TH_NORMAL_;
            th_weight = TH_WEIGHT_;
            th_lcc_normal_similiarity = TH_LCC_NORMAL_SIMILARITY_;
            th_lcc_planar_model_dist = TH_LCC_PLANAR_MODEL_DIST_;
            th_obstacle = TH_OBSTACLE_HEIGHT_;
            refine_mode = REFINE_MODE_;
            visualization = VIZ_MDOE_;
        }
